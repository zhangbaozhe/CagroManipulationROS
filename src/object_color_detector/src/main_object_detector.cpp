                    ...
                    object_color_detector::DetectObjectSrv::Response &res)
{
    ...
    if (!box.empty())
    {
        for (; box2draw < box.size(); box2draw++)
        {
            ...
            targetPose.position.x = box.at(box2draw).x + cvRound(box.at(box2draw).width / 2) + ROI_x;
            targetPose.position.y = box.at(box2draw).y + cvRound(box.at(box2draw).height / 2) + ROI_y;
            res.yellowObjList.push_back(targetPose);
            res.yellowRectX = box.at(box2draw).x;
            res.yellowRectY = box.at(box2draw).y;
            res.yellowRectXWidth = box.at(box2draw).width;
            res.yellowRectYHeight = box.at(box2draw).height;
        }
    }
...
int main(int argc, char **argv)
{
    ...
    // new added for yellow
    nh.param("yellow/hmin", hsv_object[3].hmin, 40);
    nh.param("yellow/hmax", hsv_object[3].hmax, 60);
    ...
    // new added for yellow
    hsv_object[3].color = cv::Scalar(0, 255, 255);
